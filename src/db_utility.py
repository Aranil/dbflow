...
def create_sql(sql_file='main_query_datatypes_barchart.sql', replacements=None, write_sql=True):
    ...
    res_file = pkg_resources.resource_string('db_utils.sql', sql_file).decode('utf-8')
...
def generate_internal_temp_folder(folder_name):
    f_dir = Path(db_utils.sql.__file__).parent.parent.parent.parent.joinpath(folder_name)
    try:
        f_dir.mkdir(parents=True, exist_ok=False)
    except FileExistsError:
